...
template<typename Dtype, typename Mtype>
void DataTransformer<Dtype,Mtype>::Transform(
    const AnnotatedDatum& anno_datum, Blob<Dtype,Mtype>* transformed_blob,
...
template<typename Dtype, typename Mtype>
void DataTransformer<Dtype,Mtype>::TransformAnnotation(
...
void DataTransformer<Dtype,Mtype>::Transform(const cv::Mat& cv_img,
  ...
  *crop_bbox = UnitBBox();
  ...
  Dtype* transformed_data = transformed_blob->mutable_cpu_data();
  clock_t start_clock_decode = clock();
  int top_index;
  for (int h = 0; h < height; ++h) {
    ...
    for (int w = 0; w < width; ++w) {
      ...
      for (int c = 0; c < img_channels; ++c) {
        ...
        if (has_mean_file) {
          ...
          if (has_mean_values) {
           // transformed_data[top_index] = Get<Dtype>( 
           //     (pixel - Get<Dtype>(mean_values_[c])) * scale);
            transformed_data[top_index] = Get<Dtype>(
  ...
  LOG(INFO) << "Transform:" << float((clock() - start_clock_decode) / (CLOCKS_PER_SEC / 1000) );
}

template<typename Dtype, typename Mtype>
void DataTransformer<Dtype,Mtype>::TransformInv(const Dtype* data, cv::Mat* cv_img,
  ...
  for (int h = 0; h < height; ++h) {
    ...
    for (int w = 0; w < height; ++w) {
      for (int c = 0; c < channels; ++c) {
        ...
        if (has_mean_file) {
          ptr[img_idx++] = static_cast<uchar>(Get<Mtype>(data[idx]) / scale + Get<Mtype>(mean[idx]));
        } else {
          if (has_mean_values) {
            ptr[img_idx++] =
                static_cast<uchar>(Get<Mtype>(data[idx]) / scale + Get<Mtype>(mean_values_[c]));
          } else {
            ptr[img_idx++] = static_cast<uchar>(Get<Mtype>(data[idx]) / scale);
          }
        }
      }
    }
...
template<typename Dtype, typename Mtype>
void DataTransformer<Dtype,Mtype>::TransformInv(const Blob<Dtype,Mtype>* blob,
