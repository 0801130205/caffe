...
void DetectionOutputLayer<Dtype,Mtype>::LayerSetUp(const vector<Blob<Dtype,Mtype>*>& bottom,
  ...
  if (visualize_) {
    ...
    data_transformer_.reset(
        new DataTransformer<Dtype,Mtype>(this->layer_param_.transform_param(),
                                   this->phase_));
...
template <typename Dtype, typename Mtype>
void DetectionOutputLayer<Dtype,Mtype>::Reshape(const vector<Blob<Dtype,Mtype>*>& bottom,
      const vector<Blob<Dtype,Mtype>*>& top) {
  if (need_save_) {
...
template <typename Dtype, typename Mtype>
void DetectionOutputLayer<Dtype,Mtype>::Forward_cpu(
  ...
  if (visualize_) {
    ...
    VisualizeBBox(cv_imgs, top[0]->cpu_data(), top[0]->height(), visualize_threshold_, colors,
