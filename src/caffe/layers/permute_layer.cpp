...
template <typename Dtype, typename Mtype>
void PermuteLayer<Dtype,Mtype>::LayerSetUp(const vector<Blob<Dtype,Mtype>*>& bottom,
...
template <typename Dtype, typename Mtype>
void PermuteLayer<Dtype,Mtype>::Reshape(const vector<Blob<Dtype,Mtype>*>& bottom,
      const vector<Blob<Dtype,Mtype>*>& top) {
...
template <typename Dtype, typename Mtype>
void PermuteLayer<Dtype,Mtype>::Forward_cpu(const vector<Blob<Dtype,Mtype>*>& bottom,
      const vector<Blob<Dtype,Mtype>*>& top) {
  if (need_permute_) {
    ...
    bool forward = true;
    Permute<Dtype,Mtype>(top_count, bottom_data, forward, permute_order, old_steps,
...
template <typename Dtype, typename Mtype>
void PermuteLayer<Dtype,Mtype>::Backward_cpu(const vector<Blob<Dtype,Mtype>*>& top,
  ...
  if (need_permute_) {
    ...
    const int* new_steps = new_steps_.cpu_data();
    bool forward = false;
    Permute<Dtype,Mtype>(top_count, bottom_diff, forward, permute_order, old_steps,
