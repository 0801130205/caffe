...
template <typename Dtype, typename Mtype>
void PermuteLayer<Dtype,Mtype>::Forward_gpu(const vector<Blob<Dtype,Mtype>*>& bottom,
  ...
  if (need_permute_) {
    ...
    PermuteKernel<Dtype,Mtype><<<CAFFE_GET_BLOCKS(count), CAFFE_CUDA_NUM_THREADS>>>(
...
template <typename Dtype, typename Mtype>
void PermuteLayer<Dtype,Mtype>::Backward_gpu(const vector<Blob<Dtype,Mtype>*>& top,
