...
VideoDataLayer<Dtype,Mtype>::VideoDataLayer(const LayerParameter& param)
  : BasePrefetchingDataLayer<Dtype,Mtype>(param) {
}

template <typename Dtype, typename Mtype>
VideoDataLayer<Dtype,Mtype>::~VideoDataLayer() {
...
template <typename Dtype, typename Mtype>
void VideoDataLayer<Dtype,Mtype>::DataLayerSetUp(
...
template<typename Dtype, typename Mtype>
void VideoDataLayer<Dtype,Mtype>::load_batch(Batch<Dtype,Mtype>* batch) {
  ...
  LOG(INFO) << "VideoLayer:" << float((clock() - start_clock_decode) / (CLOCKS_PER_SEC / 1000) );
}
