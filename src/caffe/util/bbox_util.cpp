...
template void GetMaxConfidenceScores(const double* conf_data, const int num,
      ...
      const int background_label_id, const ConfLossType loss_type,
      vector<vector<float> >* all_max_scores);
template void GetMaxConfidenceScores(const float16* conf_data, const int num,
      const int num_preds_per_class, const int num_classes,
      const int background_label_id, const ConfLossType loss_type,
      vector<vector<float> >* all_max_scores);
...
template <typename Dtype>
void VisualizeBBox(const vector<cv::Mat>& images, const Dtype* detections_data, 
                   const int num_det,
...
//  LOG(INFO) << typeid(detections_data).name() ;
  // Retrieve detections.
  ...
  start_clock = clock();
  LOG(INFO) << "Decode" << (clock() - start_clock_decode);
}

template
void VisualizeBBox(const vector<cv::Mat>& images,
                   const float* detections_data, const int num_det, 
                   const float threshold, const vector<cv::Scalar>& colors,
                   const map<int, string>& label_to_display_name);
template
void VisualizeBBox(const vector<cv::Mat>& images,
                   const double* detections_data, const int num_det,
                   const float threshold, const vector<cv::Scalar>& colors,
                   const map<int, string>& label_to_display_name);
template
void VisualizeBBox(const vector<cv::Mat>& images,
                   const float16* detections_data, const int num_det,
                   const float threshold, const vector<cv::Scalar>& colors,
                   const map<int, string>& label_to_display_name);
