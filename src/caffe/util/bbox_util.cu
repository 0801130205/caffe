...
template __host__ __device__ double BBoxSizeGPU(const double* bbox,
    const bool normalized);
template __host__ __device__ float16 BBoxSizeGPU(const float16* bbox,
    const bool normalized);
...
template __host__ __device__ double JaccardOverlapGPU(const double* bbox1,
    const double* bbox2);
template __host__ __device__ float16 JaccardOverlapGPU(const float16* bbox1,
    const float16* bbox2);
...
template __device__ void ClipBBoxGPU(const float16* bbox, float16* clip_bbox);
...
template void ComputeOverlappedGPU(const int nthreads,
          const float16* bbox_data, const int num_bboxes, const int num_classes,
          const float16 overlap_threshold, bool* overlapped_data);
...
template void ComputeOverlappedByIdxGPU(const int nthreads,
          ...
          const int* idx, const int num_idx, bool* overlapped_data);
template void ComputeOverlappedByIdxGPU(const int nthreads,
          const float16* bbox_data, const float16 overlap_threshold,
          const int* idx, const int num_idx, bool* overlapped_data);
...
void ApplyNMSGPU(const Dtype* bbox_data, const Dtype* conf_data,
  ...
  Blob<int, int> idx_blob(1, 1, 1, num_remain);
  int* idx_data = idx_blob.mutable_cpu_data();
  std::copy(idx.begin(), idx.begin() + num_remain, idx_data);

  Blob<bool, bool> overlapped(1, 1, num_remain, num_remain);
...
void GetDetectionsGPU(const Dtype* bbox_data, const Dtype* conf_data,
          ...
          const bool clip_bbox, Blob<Dtype,Mtype>* detection_blob) {
  // Store selected indices in array.
  int num_det = indices.size();
  if (num_det == 0) {
    return;
  }
  Blob<int,int> idx_blob(1, 1, 1, num_det);
  int* idx_data = idx_blob.mutable_cpu_data();
...
template void GetDetectionsGPU(const float* bbox_data, const float* conf_data,
          const int image_id, const int label, const vector<int>& indices,
          const bool clip_bbox, Blob<float, float>* detection_blob);
template void GetDetectionsGPU(const double* bbox_data, const double* conf_data,
          const int image_id, const int label, const vector<int>& indices,
          const bool clip_bbox, Blob<double, double>* detection_blob);
//template void GetDetectionsGPU(const float16* bbox_data, const float16* conf_data,
//          const int image_id, const int label, const vector<int>& indices,
//          const bool clip_bbox, Blob<float16, float16>* detection_blob);
